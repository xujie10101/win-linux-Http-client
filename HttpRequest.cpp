...
int HttpRequest::HttpRequestExec(const char* strMethod, const char* strUrl, const char* strData, char* strResponse)
{
	...
	u_long ulValue;
	ulValue = 1;
	if (ioctlsocket(m_iSocketFd, FIONBIO, &ulValue) != 0)
	{
		return false;
	}
#else
	...
	else
	{
		fd_set set;
		FD_ZERO(&set);
		FD_SET(m_iSocketFd, &set);

		if (select(-1, NULL, &set, NULL, &tm) <= 0)
		{
			iRet = -1; // 有错误(select错误或者超时)
		}
		else
		{
			int error = -1;
			int optLen = sizeof(int);
			getsockopt(m_iSocketFd, SOL_SOCKET, SO_ERROR, (char*)&error, &optLen); 
	...
	// 设回为阻塞socket
	ulValue = 0;
	if (ioctlsocket(m_iSocketFd, FIONBIO, &ulValue) != 0)//设置为阻塞模式
	{
		return false;
	}
#else
	iRet = connect(m_iSocketFd, (struct sockaddr *)&servaddr, sizeof(servaddr));
#endif
	if(iRet == 0) {
		char* strResult = HttpDataTransmit(strHttpHead, m_iSocketFd);
		if(NULL != strResult) {
...
char* HttpRequest::HttpDataTransmit(char *strHttpHead, const int iSockFd)
{
	...
	int ret = send(iSockFd,strHttpHead,strlen(strHttpHead)+1,0); 
	free(strHttpHead);
	if (ret < 0) { 
